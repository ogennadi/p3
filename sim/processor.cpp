...
void Processor::tick ()
{
    ...
    if (inbound_request)
    {
    	fprintf(stdout,"* COMPLETE -- PR: %d -- Clock: %lld\n",moduleID.nodeID, Global_Clock);
    ...
    if (fscanf (infile, "%c 0x%llx\n", &c, (unsigned long long int*)&addr) == 2)
    {
        ...
        addr = addr & ((~0x0) << settings.cache_line_size_log2);
        addr = addr >>settings.cache_line_size_log2;
        
        fprintf (stdout,"* FETCH -- PR: %d -- Clock: %lld -- %c 0x%llx\n", 
